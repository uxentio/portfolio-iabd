# Programación para IA

Proyectos de integración: agentes de IA, RPA + RAG, IoT con MQTT y aplicaciones cross-platform con .NET MAUI.

## Proyectos

| Carpeta | Descripción |
|---|---|
| [`agentes-ia-rpa-rag/`](agentes-ia-rpa-rag) | **Cosmic Briefing** (n8n + Telegram + Groq), **Estación IoT**, y **práctica RPA + RAG** con Playwright + ChromaDB |
| [`iot-mqtt-nodered-maui/`](iot-mqtt-nodered-maui) | Integración IoT extremo-a-extremo: sensores → MQTT → Node-RED → backend → MAUI + LLM |
| [`ejercicios-csharp-poo/`](ejercicios-csharp-poo) | Ejercicios de programación orientada a objetos en C# |
| [`tarea-llm-integration/`](tarea-llm-integration) | Memoria de integración con LLMs |

## Archivos sueltos

- `5_Ferrer_Martínez.wxmx` — Trabajo en wxMaxima
